...
public class Solution {
    ...
    public static void sort(List<Stock> list) {
        list.sort((stock1, stock2) -> {
            String a = (String)stock1.get("name");
            String b = (String)stock2.get("name");
            int x = a.compareTo(b) * 10000;
            Date aa = (Date) stock1.get("date");
            Date bb = (Date) stock2.get("date");
            int y = bb.compareTo(aa) * 1000;
            double aaa;
            double bbb;
            if (stock1.containsKey("change")) {
                aaa = (double)stock1.get("change");
            } else
                aaa = (double) stock1.get("last") - (double)stock1.get("open");
            if (stock2.containsKey("change")) {
                bbb = (double)stock2.get("change");
            } else
                bbb = (double) stock2.get("last") - (double)stock2.get("open");
            int z;
            if (aaa > bbb)
                z = -10;
            else if (aaa < bbb)
