...
public class Solution {
    public static long[] getNumbers(long N) {
        long[] result = null;
        long[] armstrongNumb = {1, 2, 3, 4, 5, 6, 7, 8, 9, 153, 370, 371, 407, 1634, 8208, 9474, 54748, 92727, 93084,
        548834, 1741725, 4210818, 9800817, 9926315, 24678050, 24678051, 88593477,
        146511208, 472335975, 534494836, 912985153, 4679307774L, 32164049650L, 32164049651L, 40028394225L, 42678290603L,
        ...
        for (int i = 0; i < armstrongNumb.length; i++) {
            if (armstrongNumb[i] < N) {
                end.add(armstrongNumb[i]);
            }
        }
        result = new long[end.size()];
